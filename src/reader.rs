...
{
  let mut workbook: Xlsx<_> = open_workbook(file_path).expect("db2md: cannot open given xlsx file");
...
fn parse_row(row: &[Data]) -> Vec<String>
{
  ...
  for cell in row.iter() {
    match cell {
      ...
      Data::String(s) | Data::DateTimeIso(s) | Data::DurationIso(s) => res.push(s.to_string()),
